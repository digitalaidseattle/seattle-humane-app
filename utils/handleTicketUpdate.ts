...
export default async function handleTicketUpdate(
  ...
  if (existingTicket.client_id != clientId)
    throw new Error(`Cannot update the ticket as the ticket is not for the client with email ${existingClient.email}`)
  if (existingTicket.pet_id != petId)
    throw new Error(`Cannot update the ticket as the ticket is not for the pet named "${existingPet.name}" of client with email ${existingClient.email}`)
