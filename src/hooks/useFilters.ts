...
export default function useFilters(items: ServiceRequestSummary[]) {
   ...
   const filterByUrgent = (i: ServiceRequestSummary) =>
      externalFilters.global_urgent ?
         i.urgent === externalFilters.global_urgent :
         passThroughFilter();
   const filterBySpecies = (i: ServiceRequestSummary) =>
      externalFilters.global_species.length > 0 ?
         externalFilters.global_species.some(o => o.includes(i.pet.species)) :
         passThroughFilter();
   const filterByName = (i: ServiceRequestSummary) =>
      externalFilters.owner_and_pet.toLowerCase() ?
         [i.client.first_name,
         i.client.last_name,
         i.pet.name]
   ...
   const areFiltersActive = useMemo(() => {
      ...
      const externalFiltersState = Object.keys(externalFilters).every(key => {
         return jsonCompare(defaultExternalFilters[key], externalFilters[key]);
      });
